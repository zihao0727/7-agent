...
export function LarkPanel() {
  ...
  const [noticeTone, setNoticeTone] = useState<"success" | "error" | "muted">("muted");
  ...
  const showNotice = (text: string, tone: "success" | "error" | "muted" = "muted") => {
    setNotice(text);
    setNoticeTone(tone);
  };
  ...
  const handleAuth = async (account: LarkAccountInfo) => {
    ...
    try {
      ...
      setAuthUrl(url);
      setDeviceCode(code);
      showNotice(url ? "授权链接已生成，完成后请回到这里确认。" : "授权已发起，请按飞书提示完成。", "success");
    } catch {
  ...
  const handleCompleteAuth = async () => {
    ...
    try {
      ...
      setDeviceCode(null);
      setAuthAccountId(null);
      showNotice("授权完成，飞书连接可用。", "success");
    } catch {
